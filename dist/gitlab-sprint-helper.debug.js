...
window.SummaryView = class SummaryView {
  ...
  // File: lib/ui/views/SummaryView.js - addAssigneeRowToElement function
...
// File: lib/ui/views/SummaryView.js - renderDataTableWithDistribution function (part that needs modification)
  ...
  async renderDataTableWithDistribution(container, assigneeTimeMap, totalHours, boardData, boardAssigneeData) {
    ...
    if (boardNames.length > 0 && boardData) {
      ...
      const distributionText = distributionValues.map((hours, index) => {
        let spanHTML = `<span style="`;
        if (index === distributionValues.length - 1 ||
...
window.SprintManagementView = class SprintManagementView {
  ...
  copySprintDataFromHistory(sprint) {
    try {
      ...
      // Convert numbers to strings with comma as decimal delimiter
      const formatNumber = (num) => num.toString().replace('.', ',');

      const formattedData = `${formatNumber(totalTickets)}\n${formatNumber(closedTickets)}\n${formatNumber(totalHours)}\n${formatNumber(totalClosedHours)}\n\n${prediction}`;
